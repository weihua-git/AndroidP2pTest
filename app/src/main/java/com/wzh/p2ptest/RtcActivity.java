...
//        callerId = "";//先用一台设备运行，然后通过日志获取id,填入id,再运行到另一台设备上
    ...
    /**
     * 客户端和有ts端的客户端连接成功发送ts文件到需要ts的客户端
     */
    public void sendTsFile(){
...
//                    appText("收到 " + new String(data));
                    ...
                    try {
                        fHelper.save("music.mp3", data);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    // 判断是否存在该文件
                    Log.e(TAG, "file size=" + data.length);
                    appText("收到 文件 fileSize" + data.length);
                    Log.e(TAG,"收到 文件 fileSize" + data.length);
...
//                Toast.makeText(getApplicationContext(), newStatus, Toast.LENGTH_SHORT).show();
    ...
    @Override
    public void onDataChannel(DataChannel dataChannel,int connStatus) {
    ...
    @Override
    public void onCallReadyCallerId(String callerId) {
        if (callerId != null) {
            try {
                answer(callerId);
            } catch (JSONException e) {
