...
public class WebRtcClient {
    ...
    private boolean tsflag = false; // 是否存在ts文件 如果存在才发送文件到其他客户端  否则不发送
    private int connectionStatus = 0; // 客户端打洞连接是否成功 0失败 1为成功 成功时初始化 onDataChannel方法中的dataChannel 执行顺序问题 后面RtcActivity中的sendTsFile方法要用到
    ...
    public interface RtcListener {
        ...
        void onDataChannel(DataChannel dataChannel, int connStatus);

        void onCallReadyCallerId(String callerId);
...
//            Log.d(TAG,"AddIceCandidateCommand peerId="+peerId+" payload="+payload);
    ...
    private class MessageHandler {
        ...
        private Emitter.Listener onAnswer = new Emitter.Listener() {
            ...
            public void call(Object... args) {
                ...
                Log.d(TAG, "onAnswer data=" + callerId);

                mListener.onCallReadyCallerId(callerId);
            }
        };
    }
    ...
    private class Peer implements SdpObserver, PeerConnection.Observer {
        ...
        public void onCreateFailure(String s) {
            Log.e(TAG, "Peer onCreateFailure " + s);

        }

        @Override
        public void onSetFailure(String s) {
            Log.e(TAG, "Peer onSetFailure " + s);

        }

        @Override
        public void onSignalingChange(PeerConnection.SignalingState signalingState) {
            Log.e(TAG, "Peer onSignalingChange " + signalingState);
        ...
        public void onIceConnectionChange(PeerConnection.IceConnectionState iceConnectionState) {
            ...
            Log.e(TAG, "Peer onIceConnectionChange " + iceConnectionState);
        }

        @Override
        public void onIceGatheringChange(PeerConnection.IceGatheringState iceGatheringState) {
            Log.e(TAG, "Peer onIceGatheringChange " + iceGatheringState);
...
//                Log.e(TAG,"onIceCandidate "+candidate);
        ...
        public void onDataChannel(final DataChannel dataChannel) {
            Log.e(TAG, "Peer onDataChannel=" + dataChannel);
            channel = dataChannel;
            mListener.onDataChannel(dataChannel, connectionStatus);
...
//            pc.addStream(localMS); //, new MediaConstraints()
            ...
            Log.e(TAG, "createDataChannel " + channel);
    ...
    public WebRtcClient(RtcListener listener, String host, EGLContext mEGLcontext) {
        ...
        iceServers.add(new PeerConnection.IceServer("stun:47.102.156.39:3478"));

        // stun 打洞服务器 turn 打洞不行后中继服务服务器
        iceServers.add(new PeerConnection.IceServer("turn:47.102.156.39:3478?transport=udp", "test", "test123"));
        iceServers.add(new PeerConnection.IceServer("turn:47.102.156.39:3478?transport=tcp", "test", "test123"));
   ...
   /*
    ...
    public void start(String name) {
        try {
            ...
            message.put("tsflag", tsflag);
            JSONArray tsfiles = new JSONArray();
            tsfiles.put("a.ts");
            tsfiles.put("b.ts");
            message.put("tsfiles", tsflag ? tsfiles : null);
            message.put("requstTs", tsflag ? "" : "a.ts");
